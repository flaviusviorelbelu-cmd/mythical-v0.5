...
-- Add/Replace this function in MainClient.lua
-- CRASH-PROOF inventory function for MainClient.lua
local function updateInventoryDisplay()
	print("[MainClient] ?? Updating inventory display...")

	-- Check if remoteObjects exists and has GetPlayerStats
	if not remoteObjects then
		print("[MainClient] ? remoteObjects is nil - cannot update inventory")
		return
	end

	if not remoteObjects.GetPlayerStats then
	...
	local success, playerData = pcall(function()
		return remoteObjects.GetPlayerStats:InvokeServer()
	end)

	if not success or not playerData then
		warn("[MainClient] ? Failed to get player data for inventory:", playerData)
		return
	end

	print("[MainClient] ?? Got data - Coins:", playerData.coins)

	-- Initialize seeds table if missing
	if not playerData.seeds then
		playerData.seeds = {}
	end

	-- Log all seeds for debugging
	print("[MainClient] ?? Seeds in inventory:")
	for seedType, amount in pairs(playerData.seeds) do
		print("   ", seedType, ":", amount)
	end

	-- Simple inventory display in chat for now (since GUI is broken)
	...
	-- Try to find and update GUI (if it exists)
	pcall(function()
